...
def send(body,recipients):
    ...
    smtpServer = STMPEmail( STMPServer= "104.47.71.202")
...
def main():
    ...
    event=openfile('status-event.json')
    global logger
    ...
    mgr=config.SiteManager()['email']
    emg=config.EmergencyServices()['email']
    shut=config.ShudownDistributionList()
    second=config.SecondaryBreathTestLocation()
